...
def top_nav(active: str = "gex"):
    st.markdown(
        ...
        <style>
          ...
          .navwrap div[data-testid="stHorizontalBlock"] {
...
STRIKES_WINDOW_DEFAULT = 20

CHART_BG = "#2b2b2b"
GRID_COLOR = "#3a3a3a"
AXIS_TEXT = "#cfcfcf"
TITLE_TEXT = "#e6e6e6"
CHART_PADDING = {"left": 20, "right": 30, "top": 25, "bottom": 20}
...
def fetch_chain(symbol: str) -> pd.DataFrame:
    ...
    try:
        ...
        raise RuntimeError(
            f"Both Cboe delayed and fallback source failed for {symbol}. "
...
def dte_days_left(expiry_str: str) -> int:
    ...
    today = pd.Timestamp.now(tz=APP_TZ).normalize().tz_localize(None)
...
# =========================
# Chart (ROTATED: vertical gamma bars)
# =========================
def render_chart(gex_all: pd.DataFrame, spot: float, chart_title: str):
    ...
    n = len(g)
    chart_h = 620 if is_mobile else 720
    bar_size = int(np.clip(18 - (n / 8), 6, 16))

    # ascending strikes from left to right
    strike_domain = (
        g.sort_values("strike_sort", ascending=True)["strike_str"]
        .astype(str)
        .tolist()
    )
    ...
    bars = alt.Chart(g).mark_bar(size=bar_size).encode(
        x=alt.X(
            ...
            axis=alt.Axis(
                ...
                tickColor=GRID_COLOR,
                domainColor=GRID_COLOR,
                labelAngle=0 if is_mobile else -25,
                labelLimit=80,
            ),
            scale=alt.Scale(domain=strike_domain, paddingInner=0.25, paddingOuter=0.12),
        ),
        y=alt.Y(
            "shares_per_1d:Q",
            axis=y_axis,
            scale=alt.Scale(domain=[-abs_max, abs_max], nice=True),
        ),
        color=alt.Color(
            "bar_color:N",
            scale=alt.Scale(domain=["pos", "neg"], range=["#00A000", "#D00000"]),
            legend=None
        ),
        tooltip=[
    ...
    zero_line = alt.Chart(pd.DataFrame({"y": [0]})).mark_rule(
        color="#8a8a8a", opacity=0.95, strokeWidth=2
    ).encode(y="y:Q")

    # spot line: use nearest displayed strike
    spot_row = g.loc[(g["strike_sort"] - spot).abs().idxmin()]
    spot_str = str(spot_row["strike_str"])

    spot_rule = alt.Chart(pd.DataFrame({"strike_str": [spot_str]})).mark_rule(
        strokeDash=[6, 6], strokeWidth=2, opacity=0.95, color="white"
    ).encode(
        x="strike_str:N"
    )
...
with left:
    ...
    with st.spinner("Updating data…"):
        ...
        exp_tbl = (
            ...
            .assign(DTE=lambda x: x["expiry_str"].apply(dte_days_left))
            .sort_values("T")
